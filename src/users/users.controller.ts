...
export class UsersController {
    ...
    @Get('/my-cars')
    async getMyCars(@GetUser('id') userId: number, @ParseQuery() query: RawQueryParamsDto): Promise<GetCarDto[]> {
        return this.usersService.getMyCars(userId, query);
    }

    @Post('/save-car')
    async saveCar(@GetUser('id') userId: number, @Body(ValidationPipe) { carId }: SaveCarDTO): Promise<object> {
        return await this.usersService.saveCar(userId, carId);
    }

    @Get('/get-saved-cars')
    async getSavedCars(@GetUser('id') userId: number, @ParseQuery() query: RawQueryParamsDto): Promise<GetCarDto[]> {
